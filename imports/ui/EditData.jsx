...
export const EditData = () => {
  ...
  const { _id } = useParams();
  const barang = useTracker(() => BarangCollection.find({ _id: _id }).fetch());
  ...
  const getbarang = () => {
    barang.map((b) => {
      setNama(b.nama);
      setHarga(b.harga);
      setJumlah(b.jml_barang);
      setDistributor(b.distributor);
    });
  }
  useEffect(() => {
    getbarang();
          ...
          <form onSubmit={handleSubmit} >
            ...
            <div className="columns">
              ...
              <div className="column is-1">
                <button type='submit' className='tombol-submit button is-link'>Edit</button>
              </div>
            </div>
